  ...
  return (
    <div className="service-page bg-white text-[#17232c]">
        ...
        <Container className="px-4 sm:px-6 lg:px-8">
          <div className="grid overflow-hidden bg-[#f6f5f2] lg:grid-cols-[0.8fr_1.2fr]">
            <div className="relative min-h-[280px] sm:min-h-[360px]">
              ...
              <div className="absolute bottom-0 left-0 h-2 w-24 bg-[#e00019]" />
            </div>
            <div className="p-7 sm:p-10 xl:p-12">
              <h2 id="when-to-involve" className="text-[#111b23] service-section-heading"><ServiceHeadingText text="When to involve us" /></h2>
              <ul className="mt-6 divide-y divide-[#e8e7e4]">
                {service.whenToInvolve.map((point, index) => <li key={point} className="flex gap-4 py-4"><span className="mt-1 font-mono text-[11px] text-[#e00019]">0{index + 1}</span><p className="text-sm leading-7 text-[#53606a]">{point}</p></li>)}
              </ul>
            </div>
          </div>
        </Container>
          ...
          <div>
            <div className="min-w-0">
              {service.subservices && <section id="areas-of-work" aria-labelledby="work-title" className="scroll-mt-28">
                <div className="mb-6 border-b border-[#e8e7e4] pb-6"><p className="mb-3 text-xs font-semibold uppercase tracking-[0.15em] text-[#e00019]">{service.title}</p><h2 id="work-title" className="text-[#111b23] service-section-heading">Our <span className="text-[#e00019]">subservices</span></h2></div>
      ...
      {relatedServices.length > 0 && <section className="py-14 sm:py-20">
        <Container className="px-4 sm:px-6 lg:px-8">
          <div className="mb-8 flex items-center justify-between gap-4"><h2 className="text-[#111b23] service-section-heading"><ServiceHeadingText text="Related services" /></h2><Link href="/services" className="inline-flex items-center gap-2 text-xs font-semibold text-[#e00019]">All services<ArrowUpRight className="h-4 w-4" aria-hidden="true" /></Link></div>
          <div className="grid gap-6 md:grid-cols-3">{relatedServices.map(related => <Link key={related.slug} href={`/services/${related.slug}`} className="group relative min-h-[280px] overflow-hidden bg-[#111b23] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#c80016]">
      ...
      <section className="bg-[#f6f5f2] py-12 sm:py-16">
        <Container className="px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col items-start justify-between gap-8 border-l-2 border-[#c80016] pl-6 sm:pl-10 lg:flex-row lg:items-center">
            <div className="max-w-3xl"><h2 className="text-[#111b23] service-section-heading">Discuss a <span className="text-[#e00019]">{service.title}</span> matter.</h2><p className="mt-4 text-sm leading-7 text-[#53606a]">Tell us about the transaction or question and we will outline the work involved.</p></div>
