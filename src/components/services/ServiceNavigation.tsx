        ...
        {[{ slug: "", number: "00", title: "All services" }, ...getServices()].map((service) => {
          ...
          return (
            <Link key={service.slug} href={service.slug ? `/services/${service.slug}` : "/services"} aria-current={active ? "page" : undefined}
              className={`flex items-start gap-3 border-b border-[#e8e7e4] px-5 py-4 text-sm leading-6 transition-colors last:border-b-0 hover:bg-[#fafaf9] focus-visible:outline-2 focus-visible:outline-[#c80016] ${active ? "border-l-2 border-l-[#e00019] bg-[#f6f5f2] font-semibold text-[#e00019]" : "text-[#17232c]"}`}>
              <span className="mt-0.5 font-mono text-[11px] text-[#63717c]">{service.number}</span>{service.title}
            </Link>
          );
        })}
      </nav>
      <div className="mt-5 border-t-2 border-[#e00019] bg-[#f6f5f2] p-6">
        ...
        <Link href="/contact" className="mt-5 inline-flex items-center gap-3 text-sm font-semibold text-[#e00019] hover:underline">Ask our team <ArrowUpRight className="h-4 w-4" aria-hidden="true" /></Link>
      </div>
    </aside>
  );
}
